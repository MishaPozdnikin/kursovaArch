...
void run(stateType);
int convertNum(unsigned long long int);
...
main(int argc, char* argv[])
{
	...
	state.mem = (unsigned long long int*)malloc(sizeof(unsigned long long int*) * NUMMEMORY);
	...
	for (state.numMemory = 0; fgets(line, MAXLINELENGTH, filePtr) != NULL;
		...
		if (sscanf(line, "%llx", state.mem + state.numMemory) != 1) {
...
run(stateType state)
{
	...
	for (; 1; instructions++) { /* infinite loop, exits when it executes halt */
		...
		opcode = state.mem[state.pc] >> 42;
		arg0 = (state.mem[state.pc] >> 34) & 0xFF;
		arg1 = (state.mem[state.pc] >> 26) & 0xFF;
		arg2 = state.mem[state.pc] & 0xFF; /* only for add, nand */

		addressField = convertNum(state.mem[state.pc] & 0x3FFFFFF); /* for beq,
										lw, sw */
		if (addressField & 0x2000000) {
			addressField = addressField | 0xFC000000;
		}
		state.pc++;
		if (opcode == ADD) {
...
int
convertNum(unsigned long long int num)
{
	/* convert a 32-bit number into a 64-bit Sun integer */
	if (num & (INT64_C( 1) << 31)) {
		num -= (INT64_C( 1) << 32);
	}
	return(num);
